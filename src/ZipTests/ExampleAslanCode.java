...
public class ExampleAslanCode {
    public static void main(String[] args) throws IOException {
        ...
        System.out.printf("%-41s (%d байт) : %s%n", "Архивируаем и сохраняем во временный файл",
                Files.size(compressedFile), compressedFile);

        Path deCompressedFile = decompressFile(compressedFile);
        System.out.printf("%-41s (%d байт) : %s%n%n", "РазАрхивировываем и получаем файл",
                Files.size(deCompressedFile), deCompressedFile);

        System.out.println("Содержимое которого :");
        Files.lines(deCompressedFile).forEach(System.out::println);

        if (Desktop.isDesktopSupported()) Desktop.getDesktop().open(deCompressedFile.toFile());
    }
    ...
    public static Path compressURLfile(String urlString) {
        ...
        try (InputStream inputStream = new URL(urlString).openStream()) {
            compressedFile = Files.createTempFile(urlString.replaceAll("(.*/)", ""), ".gz");
